...
def test_compute_coriolis_force():
    v = np.array([0.0, 100.0, 0.0])
    m = 1000.0
    F = forces.compute_coriolis_force(v, m)
    assert F.shape == (3,)
...
def test_compute_specific_forces_keys():
    ...
    for k in ['gravity', 'thrust', 'drag', 'coriolis', 'total', 'gravity_magnitude', 'thrust_magnitude', 'drag_magnitude', 'coriolis_magnitude']:
        assert k in out
