...
def rk4_step(state: State, torque: np.ndarray, dt: float,
    ...
    k1 = state_derivative_vector(y, t, torque, thrust_on, throttle, dry_mass=dry_mass, stage=stage)

    y2 = y + 0.5 * dt * k1
    y2[6:10] = quaternion_normalize(y2[6:10])
    k2 = state_derivative_vector(y2, t + 0.5*dt, torque, thrust_on, throttle, dry_mass=dry_mass, stage=stage)

    y3 = y + 0.5 * dt * k2
    y3[6:10] = quaternion_normalize(y3[6:10])
    k3 = state_derivative_vector(y3, t + 0.5*dt, torque, thrust_on, throttle, dry_mass=dry_mass, stage=stage)

    y4 = y + dt * k3
    y4[6:10] = quaternion_normalize(y4[6:10])
    k4 = state_derivative_vector(y4, t + dt, torque, thrust_on, throttle, dry_mass=dry_mass, stage=stage)
...
def euler_step(state: State, torque: np.ndarray, dt: float,
    ...
    # dy/dt = f(t, y)
    dy = state_derivative_vector(y, t, torque, thrust_on, throttle, dry_mass=dry_mass, stage=stage)
...
def integrate(state: State, torque: np.ndarray, dt: float,
              ...
              dry_mass: float = C.DRY_MASS, stage: int = 1) -> State:
    """
    ...
    """
    if method == 'rk4':
        return rk4_step(state, torque, dt, thrust_on, throttle, dry_mass, stage)
    elif method == 'euler':
        return euler_step(state, torque, dt, thrust_on, throttle, dry_mass, stage)
    else:
